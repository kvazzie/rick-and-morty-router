...
export function ItemsListContent<T extends Category>({ category }: { category: T }) {
  ...
  const [items, nextItems] = useItems(
    routesMap[category],
    (news) => (console.log('setHistory', news), setHistory((currents) => [...currents, news])),
  );

  const observer = useIntersectionObserver(
    (node) => {
      console.log('IntersectionObserver', node);
      if (node[0].isIntersecting) (console.log('Intersecting, load more...'), nextItems());
    },
    {},
    [],
  );
...
function LastBlock<T extends Category>({ promise, category, observer }: LastBlockProps<T>) {
  ...
  useEffect(() => {
    ...
    return () => {
      (console.log(observer), observer?.unobserve(target));
    };
  }, [promise, observer]);
