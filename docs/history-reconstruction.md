...
| `AGENTS.md`, the `CLAUDE.md` symlink, and `docs/agents/` | Retained on `dev` as the repository's agent guidance.                                                 |
| Deleted `GEMINI.md`                                      | Deletion retained; its npm, ESLint, and route notes were stale.                                       |
...
The retained paths came from the archive, so the temporary stash can be removed.

## Stash review

| Commit                                     | Review and decision                                                                                                                                                |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `45f29e7ed247f36532e2872c09861c51d0657a2f` | Early Husky and Commitlint work. The useful configuration was already integrated and is represented by reconstructed commit `c118ae7`; remove the duplicate stash. |
...
| `5491538ab9f60adca1dc923c3a070fb908a6ab30` | Duplicate of the preceding Oxfmt experiment. Both have stable patch ID `c995f6e662a803387562c95995feb38a5af882c6`; remove it.                                      |
| `92d24873259a262ca3eb4c34bff1800da88d0b3b` | Adds `dev-dist` to `.gitignore`. Its patch ID matches reconstructed commit `6d368f9`; remove the duplicate stash.                                                  |
...
moved to Oxlint and Oxfmt. The branch is obsolete and can be removed after this
record is committed.
...
original tips, so those local names can also be removed.
...
Before the retained worktree files and these records were added, the tree at
`937a114` matched the original unpublished tree at `8e06534` exactly. The
reconstruction drops only the old merge structure, duplicated stashes, and
recorded obsolete experiments.
